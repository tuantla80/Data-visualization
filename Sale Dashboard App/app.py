...
app.layout = html.Div([
    ...
    # Second row
    html.Div([
        # Horizontal bar chart with radio items for filtering data
        html.Div([
            # Radio items
    ...
    html.Div([
        ...
        html.Div([
            dt.DataTable(id='my_datatable',
                         columns = [{'name': i, 'id': i} for i in  sales.columns],  # column names to display
                         virtualization=True,  # display large dataset by scrolling, pagination
                         style_cell={'textAlign': 'left',
                                     'min-width': '100px',
                                     'backgroundColor': '#1f2c56',
                                     'color': '#FEFEFE',
                                     'border-bottom': '0.01rem solid #19AAE1'},  # styling for each cell
                         style_header={'backgroundColor': '#1f2c56',
                                       'fontWeight': 'bold',
                                       'font': 'Lato, sans-serif',
                                       'color': 'orange',
                                       'border': '#1f2c56'},  # styling for header (column names)
                         style_as_list_view=True,
                         style_data={'styleOverflow': 'hidden', 'color': 'white'},
                         fixed_rows={'headers': True},  # fixed header (column names)
                         sort_action='native',  # add sort button
                         sort_mode='multi')
        ...
        # Bubble chart
        html.Div([dcc.Graph(id='bubble_chart', config={'displayModeBar': 'hover'},)
...
def update_barchart_1(select_years, radio_items, radio_items1):
    ...
    sales2 = sales1[(sales1['Year'] == select_years) & (sales1['Segment'] == radio_items)]\
                ...
                .nlargest(7, columns=['Sales'])
    sales3 = sales.groupby(['Year', 'Region', 'Segment'])['Sales'].sum().reset_index()
    sales4 = sales3[(sales3['Year'] == select_years) & (sales3['Segment'] == radio_items)]\
                .sort_values(by=['Sales'], ascending=False)
...
@app.callback(Output('donut_chart', 'figure'),
              [Input('select_years', 'value')],
              [Input('radio_items', 'value')])
def update_donut_chart(select_years, radio_items):
    ...
    return {
        'data': [go.Pie(
            ...
            hole=.7,
            rotation=160, # insidetextorientation= 'radial'
        )],
        'layout': go.Layout(
...
@app.callback(Output('line_chart', 'figure'),
              [Input('select_years', 'value')],
              [Input('radio_items', 'value')])
def update_line_chart(select_years, radio_items):
...
@app.callback(Output('my_datatable', 'data'),
              [Input('select_years', 'value')],
              [Input('radio_items', 'value')])
def update_my_datatable(select_years, radio_items):
...
def update_bar_chart_2(select_years, radio_items, radio_items2):
    sales12 = sales.groupby(['Year', 'State', 'Segment'])['Sales'].sum().reset_index()
    sales13 = sales12[(sales12['Year'] == select_years) & (sales12['Segment'] == radio_items)]\
                    .sort_values(by=['Sales'], ascending=False)\
                    .nlargest(10, columns=['Sales'])
    sales14 = sales.groupby(['Year', 'City', 'Segment'])['Sales'].sum().reset_index()
    sales15 = sales14[(sales14['Year'] == select_years) & (sales14['Segment'] == radio_items)]\
                .sort_values(by=['Sales'], ascending=False)\
                .nlargest(10, columns=['Sales'])
...
@app.callback(Output('bubble_chart', 'figure'),
              [Input('select_years', 'value')],
              [Input('radio_items', 'value')])
def update_bubble_chart(select_years, radio_items):
    ...
    return {
        'data': [
            go.Scatter(  # Bubble chart is scatter plot at which the size of marker depends on a factor
                x=sales18['Month'],
                y=sales18['Sales'],
                mode='markers',
                line=dict(width=3, color='orange'),
